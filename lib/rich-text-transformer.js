...
class RichTextTransformer {
  ...
  constructor(assetMap = {}, entryMap = {}, options = {}) {
    ...
    this.urlAssetMap = options.urlAssetMap || {};
    this.options = {
  ...
  transform(wpHtml) {
    ...
    // Wrap stray inline nodes (text, hyperlink, etc.) in paragraphs
    // Contentful requires all top-level nodes to be block types
    const finalContent = this.wrapInlineNodes(content);

    // Ensure we have at least one block
    if (finalContent.length === 0) {
      finalContent.push(this.emptyParagraph());
    }
  ...
  /**
   * Wrap stray inline nodes in paragraphs so all top-level nodes are block types.
   * Consecutive inline nodes are grouped into a single paragraph.
   */
  wrapInlineNodes(nodes) {
    const BLOCK_TYPES = new Set([
      BLOCKS.DOCUMENT, BLOCKS.PARAGRAPH,
      BLOCKS.HEADING_1, BLOCKS.HEADING_2, BLOCKS.HEADING_3,
      BLOCKS.HEADING_4, BLOCKS.HEADING_5, BLOCKS.HEADING_6,
      BLOCKS.UL_LIST, BLOCKS.OL_LIST, BLOCKS.HR,
      BLOCKS.QUOTE, BLOCKS.TABLE,
      BLOCKS.EMBEDDED_ASSET, BLOCKS.EMBEDDED_ENTRY,
    ...
    const flushInline = () => {
      if (inlineBuf.length > 0) {
        result.push({ nodeType: BLOCKS.PARAGRAPH, data: {}, content: inlineBuf });
        inlineBuf = [];
      }
    };
    ...
    switch (tag) {
      ...
      case 'p':
        if (node.querySelector('img')) {
          return this.splitBlockOnImages(node.childNodes);
        }
  ...
  /**
   * Split a paragraph that contains <img> elements into a sequence of
   * paragraphs and embedded-asset-block nodes. Contentful Rich Text only
   * allows images as block-level nodes, so they can't live inside a paragraph.
   */
  splitBlockOnImages(childNodes) {
    const results = [];
    let inlineBuf = [];

    const flushInline = () => {
      const hasContent = inlineBuf.some(n =>
        (n.nodeType === 'text' && n.value.trim()) || n.nodeType !== 'text'
      );
      if (inlineBuf.length > 0 && hasContent) {
        results.push({
          nodeType: BLOCKS.PARAGRAPH,
          data: {},
          content: [...inlineBuf]
    ...
    const processChild = (child) => {
      if (child.nodeType === 3) {
        ...
        if (text) inlineBuf.push(this.createTextNode(text));
        return;
      }
      ...
      if (tag === 'img') {
        flushInline();
        const asset = this.createEmbeddedAsset(child);
        if (asset) results.push(asset);
      } else if (tag === 'a' && child.querySelector('img')) {
        flushInline();
        const img = child.querySelector('img');
        const asset = this.createEmbeddedAsset(img);
        if (asset) results.push(asset);
      } else if (['strong', 'b', 'em', 'i', 'u', 'code', 'mark'].includes(tag)) {
        const marked = this.wrapWithMark(child, this.getMarkType(tag));
        if (Array.isArray(marked)) inlineBuf.push(...marked);
        else if (marked) inlineBuf.push(marked);
      } else if (tag === 'a') {
        inlineBuf.push(this.createHyperlink(child));
      } else if (tag === 'br') {
        inlineBuf.push(this.createTextNode('\n'));
      } else if (child.querySelector && child.querySelector('img')) {
        flushInline();
        this.splitBlockOnImages(child.childNodes).forEach(n => {
          if (n) results.push(n);
        });
      } else {
        const nested = this.processInlineContent(child);
        inlineBuf.push(...nested);
      }
    };
  ...
  createEmbeddedAsset(imgNode) {
    ...
    // Resolve lazy-loaded images where src is a placeholder
    if (!src || src.startsWith('data:') || src.includes('placeholder')) {
      src = imgNode.getAttribute('data-src')
        || imgNode.getAttribute('data-lazy-src')
        || imgNode.getAttribute('data-orig-file')
    ...
    // Look up Contentful asset ID by WP media ID first
    let contentfulAssetId = wpId ? this.assetMap[wpId] : null;

    // Fallback: try URL-based lookup (handles scraped content without WP attributes)
    if (!contentfulAssetId && src) {
      const normalizedSrc = src.replace(/^https?:/, '').split('?')[0];
      for (const [url, assetId] of Object.entries(this.urlAssetMap)) {
        const normalizedUrl = url.replace(/^https?:/, '').split('?')[0];
        if (normalizedUrl === normalizedSrc || normalizedUrl.endsWith(normalizedSrc.split('/').pop())) {
